...
const ttfToWoff = () => { //TODO
    return gulp
        .src("src/fonts/**/*.ttf")
        .pipe(woff())
        .pipe(gulp.dest(path.src.fonts));
}
const ttfToWoff2 = () => { //TODO
    return gulp
        .src('src/fonts/**/*.ttf')
        .pipe(woff2())
        .pipe(gulp.dest(path.src.fonts));
...
const svgSprite = (name) => {
    const config = {
        mode: {
            stack: {
                sprite: `${path.build.img}/sprite-${name}.svg`,
            }
        }
    };
    return gulp
        .src(path.src.img + SPRITE_FILES, { allowEmpty: true }) //Выберем каталог svg изображений
...
const watcher = () => {
    ...
    gulp.watch(path.watch.img, gulp.series(gulp.parallel(bitMapImage, svgImage, svgSprite)), reload);
